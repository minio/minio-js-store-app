...
Set up a route for '/' in the minio-store.js file. Using the [listObjects](https://github.com/minio/minio-js/blob/master/examples/list-objects.js) method, get a list of all the files from the minio-store bucket. listObjects returns product urls which are pushed into an array variable called assets. Pass the assets array to `home.handlebars` view.
